...
async function submitOrder(event) {
  ...
  fields.forEach((field) => {
    ...
    if (!field.value.trim()) {
      ...
      if (!phoneRegex.test(field.value.trim())) {
        errorMsg = "Introduce un número de WhatsApp válido. (Ej: 3108876543)";
        hasError = true;
      }
    } else if (!field.checkValidity()) {
      errorMsg = "Digite un valor válido.";
      hasError = true;
    }
  ...
  // form = event.target; (já declarado acima)
...
function showSuccess(orderData) {
  ...
  const successMessageContent = document.querySelector(
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  let hoje = new Date();
  let dataMin = new Date(hoje);
  dataMin.setDate(hoje.getDate() + 1);
  let dataMax = new Date(hoje);
  dataMax.setDate(hoje.getDate() + 6);
  let texto =
    "Se entrega entre el: " +
    dataMin.getDate() +
    " " +
    meses[dataMin.getMonth()] +
    " al " +
    dataMax.getDate() +
    " " +
    meses[dataMax.getMonth()];
